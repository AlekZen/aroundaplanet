...
const WHATSAPP_CONTACT_NUMBER = '523331741585'
...
export function ConversionForm({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.success('Tu cotizacion fue registrada — te contactaremos pronto')
      onClose()
    } catch (error) {
      ...
      {availableDepartures.length === 0 ? (
        <div className="space-y-3 text-center">
          ...
          <Button asChild variant="outline" className="w-full">
            <a href={`https://wa.me/${WHATSAPP_CONTACT_NUMBER}`} target="_blank" rel="noopener noreferrer">
        ...
        <>
          ...
          <p className="text-center text-xs text-muted-foreground">
            Al confirmar, aceptas que te contactemos para darte seguimiento a tu cotizacion
          </p>
        </>
      )}
    </div>
      ...
      <div className="lg:hidden">
        <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto rounded-t-2xl">
            <SheetHeader>
              <SheetTitle>Cotizar Viaje</SheetTitle>
              <SheetDescription>Completa tus datos y selecciona una fecha</SheetDescription>
            </SheetHeader>
            {formContent}
          </SheetContent>
        </Sheet>
      ...
      <div className="hidden lg:block">
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
          <DialogContent className="max-w-[440px]">
            <DialogHeader>
              <DialogTitle>Cotizar Viaje</DialogTitle>
              <DialogDescription>Completa tus datos y selecciona una fecha</DialogDescription>
            </DialogHeader>
            {formContent}
          </DialogContent>
        </Dialog>
